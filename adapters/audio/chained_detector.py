...
from adapters.audio.steps import AiParserStep, DeterministicFallbackStep, StrictParserStep
...
class ChainedAudioModeDetector(AudioModeDetector):
    ...
    """
    def __init__(self, steps: Sequence[DetectionStep] | None = None):
        if steps is None:
            self._steps = [
                StrictParserStep(),
                AiParserStep(),
    ...
    def detect(self, wavs: list[WavInfo]) -> dict[str, list[WavInfo]]:
        ...
        for step in self._steps:
            stop_chain = step.process(processing_wavs)  # type: ignore
            if stop_chain:
                break
