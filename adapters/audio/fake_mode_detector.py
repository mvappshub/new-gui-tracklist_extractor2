...
class FakeAudioModeDetector(AudioModeDetector):
    ...
    def detect(self, wavs: list[WavInfo]) -> dict[str, list[WavInfo]]:
        ...
        for _side, wav_list in side_map.items():
    ...
    def _parse_filename(self, filename: str) -> tuple[str, int | None]:
        ...
        side_match = re.search(r"(?i)side[_-]?([A-Za-z]+)", name)
        if side_match:
            side = side_match.group(1).upper()
        else:
            # Try letter-number prefix like "A1", "AA02"
            prefix_match = re.match(r"^([A-Za-z]+)0*([1-9][0-9]?)", name)
            side = prefix_match.group(1).upper() if prefix_match else "A"

        # Parse position using regex patterns
        position = None
        
        # Try prefix number like "01_Track"
        pos_match = re.match(r"^0*([1-9][0-9]?)", name)
        if pos_match:
            position = int(pos_match.group(1))
        else:
            # Try letter-number pattern like "A1", "AA02"
            letter_num_match = re.match(r"^([A-Za-z]+)0*([1-9][0-9]?)", name)
            if letter_num_match:
                position = int(letter_num_match.group(2))
            else:
                # Try side-specific pattern like "Side_A_01", "SideA_02"
                if side_match:
                    side_pos_match = re.search(rf"(?i)side[^A-Za-z0-9]*{re.escape(side)}[^0-9]*0*([1-9][0-9]?)", name)
                    if side_pos_match:
    ...
    def _normalize_positions(self, side_map: dict[str, list[WavInfo]]) -> None:
        ...
        for _side, wav_list in side_map.items():
            ...
            # Sort by (position or 999, filename) for deterministic ordering
            wav_list.sort(key=lambda x: (x.position if x.position is not None else 999, x.filename.lower()))
            ...
            # Renumber if any position is None, duplicates exist, or sequence doesn't match expected
